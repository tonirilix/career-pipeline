...
### Requirement: sqlc query files and generated code
The system SHALL define SQL queries in `.sql` files under `internal/infrastructure/persistence/queries/`. Running `sqlc generate` SHALL produce type-safe Go functions in `internal/infrastructure/persistence/db/`.

#### Scenario: Generated code compiles without modification
- **WHEN** `go build ./...` is run after sqlc generate
- **THEN** all generated files compile without errors

### Requirement: Repository adapter implementations
The system SHALL implement all repository port interfaces (defined in `internal/application/ports/`) with adapter structs in `internal/infrastructure/persistence/`. Each adapter SHALL use sqlc-generated functions and map row types to domain structs.

#### Scenario: Domain structs have no sqlc imports
- **WHEN** the domain package is compiled
- **THEN** it SHALL contain no imports from `internal/infrastructure/persistence/db`
